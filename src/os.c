...
static int hit_time = 0;
static int miss_time = 0;
	...
	while (1) {
		...
		if (proc == NULL) {
			...
			printf("\tCPU %d: Processed %2d has finished\n",
				id ,proc->pid);
			hit_time += proc->stat_hit_time;
			miss_time += proc->stat_miss_time;
			end_proc(&proc);
			proc = get_proc();
			time_left = 0;
		}else if (time_left == 0) {
...
#endif
#ifdef CPU_TLB
		proc->stat_hit_time = 0;
		proc->stat_miss_time = 0;
		proc->tlb = &tlb;
#endif
